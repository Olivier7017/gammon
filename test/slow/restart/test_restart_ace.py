...
from gammon.calculator import AceCalc

from ...conftest import find_data_path, has_lammps_pace
from ... import context  # noqa


@pytest.mark.skipif(not has_lammps_pace(),
                    reason="LAMMPS not compiled with PACE package")
def test_restart_ace(root):
    datapath = find_data_path(root)
    struct_fn = datapath / "Nd3MgNi14-2H.cif"
    ace_fn = datapath / "Potential" / "Nd3MgNi14H05.yace"
    ...
    irr_sites = [[0.032956852013, 0.26647842601, 0.88404693872],
                 [0.421, 0.238, 0.991]]  # xred
    chem_pot = -15.5
    T = 300

    # Prepare the object
    calc = AceCalc(model_fn=ace_fn,
                   lmp_cmd="lmp_serial")
    ...
    with open(gcmc1.result_fn / "GCMC.out") as f:
        lines1 = f.readlines()
    ...
    with open(gcmc2.result_fn / "GCMC.out") as f:
        lines2 = f.readlines()
    os.chdir("..")
